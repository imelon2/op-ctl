...
var (
	epdBadgeIn  = theme.BadgeBase.Background(theme.ColorPrimary)
	epdBadgeOut = theme.BadgeBase.Background(theme.ColorOut)
)
...
func (s executionPeerDetailScreen) renderHeader() string {
	title := theme.Title.Render("execution peer detail")
	dirBadge := epdBadgeOut.Render(" outbound ")
	if s.peer.Network.Inbound {
		dirBadge = epdBadgeIn.Render(" inbound ")
	}
	src := theme.Subtitle.Render("from " + s.backend.Name + " · " + s.backend.ExecutionRPCURL)
...
func (s executionPeerDetailScreen) renderBody() string {
	...
	b.WriteString("\n")
	if s.name != "" {
		b.WriteString(detailField("namespace name", theme.Name.Render(s.name)))
	} else {
		b.WriteString(detailField("namespace name", theme.Mute.Render("(unknown)")))
	}
	b.WriteString(detailField("id", theme.Value.Render(s.peer.ID)))
	b.WriteString(detailField("name", theme.Value.Render(s.peer.Name)))
	b.WriteString(detailField("enode", theme.Value.Render(s.peer.Enode)))

	b.WriteString("\n")
	b.WriteString(theme.Section.Render("network") + "\n")
	b.WriteString(detailField("  localAddress", theme.Value.Render(s.peer.Network.LocalAddress)))
	b.WriteString(detailField("  remoteAddress", theme.Value.Render(s.peer.Network.RemoteAddress)))
	b.WriteString(detailField("  inbound", theme.Value.Render(fmt.Sprintf("%t", s.peer.Network.Inbound))))
	b.WriteString(detailField("  trusted", theme.Value.Render(fmt.Sprintf("%t", s.peer.Network.Trusted))))
	b.WriteString(detailField("  static", theme.Value.Render(fmt.Sprintf("%t", s.peer.Network.Static))))

	b.WriteString("\n")
	b.WriteString(theme.Section.Render(fmt.Sprintf("caps (%d)", len(s.peer.Caps))) + "\n")
	if len(s.peer.Caps) == 0 {
		b.WriteString("  " + theme.Mute.Render("(none)") + "\n")
	} else {
		for _, c := range s.peer.Caps {
			b.WriteString("  " + theme.Value.Render(c) + "\n")
		}
	}

	if len(s.peer.Protocols) > 0 {
		b.WriteString("\n")
		b.WriteString(theme.Section.Render(fmt.Sprintf("protocols (%d)", len(s.peer.Protocols))) + "\n")
		...
		sort.Strings(keys)
		for _, k := range keys {
			b.WriteString("  " + theme.Label.Render(k) + "\n")
...
func detailField(key, val string) string {
	return "  " + theme.Label.Render(padRight(key, 17)) + "  " + val + "\n"
}

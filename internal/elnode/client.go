// Package elnode is a thin JSON-RPC 2.0 client for the execution-layer
// admin namespace exposed by op-geth / op-erigon / geth.
//
// Why not go-ethereum/rpc: v1 calls one method (admin_nodeInfo) over plain
// HTTP POST. go-ethereum/rpc would pull in subscription/IPC/secp256k1,
// several MB and a CGO build surface, for zero benefit. This mirrors the
// rationale already documented in internal/opnode/client.go.
package elnode
...
// Self calls admin_nodeInfo on the given execution-layer URL, honoring
// ctx deadline.
//
// Returns parsed NodeInfo and the wall-clock duration of the HTTP round
...
//  8. Empty / null result      → (nil, latency, err)
//  9. Decode result error      → (nil, latency, err)
//
// 10. Success                  → (&info, latency, nil)
func Self(ctx context.Context, hc *http.Client, url string) (*NodeInfo, time.Duration, error) {
	if hc == nil {
	...
	if err := json.Unmarshal(env.Result, &info); err != nil {
		return nil, latency, fmt.Errorf("decode NodeInfo: %w", err)
	}
